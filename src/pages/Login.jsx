...
const Login = () =>{
    ...
    const login = async ()=>{
        ...
        }).then((response)=>response.json()).then((data)=> responsesData=data)

        if(responsesData.success){
            localStorage.setItem('auth-token', responsesData.token);
            window.location.replace('/');
        }else {
            alert(responsesData.errors)
        }
    }
    ...
    return (
        <section className="max-padd-container flexCenter flex-col pt-32 bg-primary">
            ...
            {state === 'Sign Up' ? <p className="text-black font-bold">Already have an account? <span onClick={()=>
            {setState("Login")}} className="text-secondary underline cursor-pointer">Login</span></p>:<p className="text-black font-bold" >Create an account?<span onClick={()=>{setState("Sign Up")}} className="text-secondary underline cursor-pointer">Click here</span></p>}
